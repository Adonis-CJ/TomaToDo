...
object Motion {
    ...
    // 时钟数字过渡（v1.6）：取消分瓣翻页，改「淡出 → 换值 → 淡入」，单值可见杜绝重影
    const val DURATION_FLIP_FADE = 140

    fun <T> flipFadeOut() = tween<T>(DURATION_FLIP_FADE, easing = EaseExit)

    fun <T> flipFadeIn() = tween<T>(DURATION_FLIP_FADE, easing = EaseEnter)
