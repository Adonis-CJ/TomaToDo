...
private fun PomodoroSession.toJson() = JSONObject()
    .put("id", id).put("taskId", taskId ?: JSONObject.NULL).put("type", type.name)
...
private fun KnowledgeCard.toJson() = JSONObject()
    ...
    .put("source", source ?: JSONObject.NULL).put("masteryLevel", masteryLevel)
    .put("reviewCount", reviewCount).put("nextReviewAt", nextReviewAt)
    .put("lastReviewedAt", lastReviewedAt ?: JSONObject.NULL).put("createdAt", createdAt)
...
private fun JSONObject.toCard(): KnowledgeCard = KnowledgeCard(
    ...
    source = optNullableString("source"), masteryLevel = optInt("masteryLevel"),
    reviewCount = optInt("reviewCount"), nextReviewAt = optLong("nextReviewAt"),
    lastReviewedAt = optNullableLong("lastReviewedAt"), createdAt = optLong("createdAt")
)
