...
- **🎯 自定义番茄钟** — 专注/短休/长休时长与长休间隔全可调；前台服务保证锁屏计时不中断；**悬浮小窗**在看别的资料时也能掌控计时；多种提示音 + 独立音量 + **静音仅震动**（图书馆友好）；阶段完成**全屏醒目提醒**（前台脉冲浮层 / 后台循环响铃通知）
- **📋 Todo 看板** — 任务卡片按「待办 / 进行中 / 已完成」三列展示，三态视觉语言（色条 + 勾选框 + 划线）一眼可辨；**日期导航**回看历史、预排未来；今日完成进度一目了然；滑动删除可撤销；一键从卡片启动番茄并自动关联
- **🗂 知识卡片** — Markdown 笔记系统：**专属撰写页**支持 Markdown + LaTeX 公式 + 表格渲染、拍照/相册插图（自动压缩、全屏缩放查看）、**标签**管理、来源、阅读↔编辑双视角、宽屏分栏实时预览、**回收站**防误删（30 天可恢复）
- **🔁 遗忘曲线复习** — 艾宾浩斯间隔（1/2/4/7/15/30 天）按「忘记/模糊/记得」动态排程；单卡沉浸刷题模式（问/答面 Markdown 渲染）；复习历史留痕
...
| 渲染 | Markwon + JLatexMath + 自研 Coil2 图片插件（Markdown/LaTeX） |
| 图片 | CameraX 契约拍照 + Photo Picker + Coil |
| 构建 | AGP 9 / Gradle 9.1 / KSP |

**设计**：[PRD.md](PRD.md)（产品需求文档）· **开发规范**：[docs/DEVELOPMENT_GUIDELINES.md](docs/DEVELOPMENT_GUIDELINES.md) · **Agent 协作**：[AGENTS.md](AGENTS.md)
...
├── data/              # Room 实体/DAO、CardRepository（卡片唯一写入口）、DataStore、备份
├── timer/             # 计时控制器、前台服务、悬浮窗、完成提醒通知
├── ui/
│   ├── board/         # 看板（三态卡片、日期导航）
│   ├── cards/         # 知识卡片（详情/编辑、回收站、render/ Markdown 渲染）
│   ├── review/        # 复习（刷题模式）
│   ├── stats/         # 统计（Canvas 自绘图表）
│   ├── settings/      # 设置（分组卡片）
│   └── theme/         # 墨·纸设计令牌、字体与动效令牌（Motion.kt）
└── ...
```
